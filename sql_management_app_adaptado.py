...
st.set_page_config(page_title='Gestão de BDOs',page_icon=r"models\favicon.png")
...
def analytics_page():
    ...
    sonda_list = list(df['OPE_COL_2'].iloc[-100:].dropna())
    st.info('Sondas mais utilizadas: ' + str(most_frequent(sonda_list)))
    ...
    st.subheader("Última operação registrada")
...
st.sidebar.subheader('Carregar BDO:')

#Uploading BDO file
uploaded_file = st.sidebar.file_uploader(label=" ", accept_multiple_files=False, label_visibility="collapsed")

bdo_button = st.sidebar.button(label='Inserir BDO!', key='bdo_button')

#Format and insert bdo
if bdo_button == True:
    bdo_df=pd.read_excel(uploaded_file)
    bdo_format_insert()
    st.sidebar.write("BDO Carregado!")
